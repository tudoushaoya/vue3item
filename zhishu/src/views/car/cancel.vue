...
const emit = defineEmits(['update:cancelVisible'])
...
const handleOk = async () => {
  ...
  await props.delfn(props.delId)

  confirmLoading.value = false
  emit('update:cancelVisible', false)
  props.refreshfn().then(() => {
...
<template>
  <div>
    <a-modal
      v-model:visible="props.cancelVisible"
      :confirm-loading="confirmLoading"
      @ok="handleOk"
      @cancel="emit('update:cancelVisible', false)"
    >
      <p style="display: flex; align-items: center">
        ...
        <span style="margin-left: 10px"> {{ ' 是否确认删除当前计费规则?' }}</span>
      </p>
    </a-modal>
  </div>
</template>
